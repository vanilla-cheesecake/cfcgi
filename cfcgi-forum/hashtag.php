<?php 
	...
	if(isset($_GET['hasshtag']) && !empty($_GET['hasshtag'])){
		$hasshtag  = $getFromU->checkInput($_GET['hasshtag']);
		$user_id  = @$_SESSION['user_id'];
		$user     = $getFromU->userData($user_id);
		$hashtags   = $getFromT->getHashtagsByHash($hasshtag);
		$accounts = $getFromT->getUsersByHash($hasshtag);
...
<html>
	<head>
		<title>#<?php echo $hasshtag;?> Hashtag on Hashtag</title>
		<meta charset="UTF-8" />
	...
	<div class="nav-container">
		...
		<div class="nav-right">
			...
			<?php if($getFromU->loggedIn() === true){?>
				...
				<li><label class="addHashtagBtn" for="pop-up-hashtag">Hashtag</label></li>
				<?php } else{
...
<div class="hash-menu">
	<div class="hash-menu-inner">
		<ul>
 			<li><a href="<?php echo BASE_URL.'hasshtag/'.$hasshtag;?>">Latest</a></li>
			<li><a href="<?php echo BASE_URL.'hasshtag/'.$hasshtag.'?f=users';?>">Accounts</a></li>
			<li><a href="<?php echo BASE_URL.'hasshtag/'.$hasshtag.'?f=photos';?>">Photos</a></li>
  		</ul>
	</div>
</div>
<!--hash-menu-->
 ...
 <div class="hash-img-wrapper"> 
 	...
 	<?php 
 		foreach ($hashtags as $hashtag) {
			...
			$rehashtag      = $getFromT->checkRehashtag($hashtag->hashtagID, $user_id);
			$user         = $getFromT->userData($hashtag->rehashtagBy);
   			
 			if(!empty($hashtag->hashtagImage)){
 			echo '<div class="hash-img-flex">
				 	...
				 	<div class="hash-img-flex-footer">
				 		...
				 		'.(($getFromU->loggedIn()) ?   '
									...
									<li>'.(((isset($rehashtag['rehashtagID'])) ? $hashtag->hashtagID === $rehashtag['rehashtagID'] OR $user_id === $rehashtag['rehashtagBy'] : '') ? '<button class="retweeted" data-tweet="'.$tweet->tweetID.'" data-user="'.$tweet->tweetBy.'"><i class="fa fa-retweet" aria-hidden="true"></i><span class="rehashtagsCount">'.(($tweet->likesCount > 0) ? $tweet->likesCount : '').'</span></button>' : '<button class="retweet" data-tweet="'.$tweet->tweetID.'" data-user="'.$tweet->tweetBy.'"><i class="fa fa-retweet" aria-hidden="true"></i><span class="rehashtagsCount">'.(($tweet->retweetCount > 0) ? $tweet->retweetCount : '').'</span></button>').'</li>
									<li>'.(((isset($likes['likeOn'])) ? $likes['likeOn'] == $hashtag->hashtagID : '') ? '<button class="unlike-btn" data-hashtag="'.$hashtag->hashtagID.'" data-user="'.$tweet->tweetBy.'"><i class="fa fa-heart" aria-hidden="true"></i><span class="likesCounter">'.(($tweet->likesCount > 0) ? $tweet->likesCount : '').'</span></button>' : '<button class="like-btn" data-tweet="'.$tweet->tweetID.'" data-user="'.$tweet->tweetBy.'"><i class="fa fa-heart-o" aria-hidden="true"></i><span class="likesCounter">'.(($tweet->likesCount > 0) ? $tweet->likesCount : '').'</span></button>').'</li>
								...
								' : '
									<li><button><i class="fa fa-share" aria-hidden="true"></i></button></li>	
									<li><button><i class="fa fa-rehashtag" aria-hidden="true"></i></button></li>	
	...
	<div class="follow-unfollow-inner">
		...
		<div class="follow-person-bio">
			...
			<div class="follow-person-dis">
			    <?php echo $getFromT->getHashtagLinks($users->bio);?>
			</div>
		</div>
	</div>
</div>
		...
		<div class="in-center-wrap">
		  <?php 
			...
			$user         = $getFromT->userData($hashtag->rehashtagBy);
   			...
   			echo '<div class="all-hashtag">
					<div class="t-show-wrap">	
					 <div class="t-show-inner">
   							'.(((isset($rehashtag['rehashtagID'])) ? $rehashtag['rehashtagID'] === $hashtag->rehashtagID or $hashtag->rehashtagID > 0 : '') ? ' 
 							<div class="t-show-popup" data-hashtag="'.$hashtag->hashtagID.'" data-user="'.$hashtag->hashtagBy.'">
 							<div class="t-show-banner">
								<div class="t-show-banner-inner">
									<span><i class="fa fa-rehashtag" aria-hidden="true"></i></span><span>'.$user->screenName.'. Rehashtagged</span>
								</div>
							</div>' : '').'
					 		 '.(((isset($rehashtag['rehashtagMsg'])) ? !empty($retweet['retweetMsg']) && $tweet->tweetID === $retweet['tweetID'] or $tweet->retweetID > 0 : '') ? '<div class="t-show-head">
